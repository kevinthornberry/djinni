...
import djinni.ast.{Doc, Field, Ident, Interface, ProtobufMessage, Record, TypeDecl, TypeDef, TypeParam, TypeRef}
...
class CGenerator(spec: Spec) extends Generator(spec) {
  ...
  def implHeader(t: TypeDecl): String = implHeader(t.body, t.ident)

  def implHeader(td: TypeDef, ident: Ident): String = {
    td match {
      case Interface(_, _, _) | Record(_, _, _, _) =>
        q(spec.cIncludePrefix + ident.name + "_impl.hpp")
      case _ => ""
    }
  }
  ...
  private def writeCFile(origin: String, name: String, ext: String, targetDir: File, f: IndentWriter => Unit): Unit = {
    createFile(targetDir, name + "." + ext, (w: IndentWriter) => {
  ...
  private def writeCFilePair(origin: String, ident: Ident, publicIncludes: Seq[String], privateIncludes: Seq[String], td: ast.TypeDef)(header: IndentWriter => Unit, impl: IndentWriter => Unit): Unit = {
    writeCFile(origin, ident.name, "h", spec.cHeaderOutFolder.get, (w: IndentWriter) => {
      w.wl("#pragma once")
      w.wl
    ...
    writeCFile(origin, ident.name, "cpp", spec.cOutFolder.get, (w: IndentWriter) => {
      w.wl(s"""#include ${publicHeader(ident)}""")

      td match {
        case Interface(_, _, _) | Record(_, _, _, _) =>
          w.wl(s"""#include ${implHeader(td, ident)}""")
        case _ =>
      }
  ...
  override def generateEnum(origin: String, ident: Ident, doc: Doc, e: ast.Enum): Unit = {
    writeCFilePair(origin, ident, List.empty[String], List.empty[String], e)((w: IndentWriter) => {
  ...
  private def generateRefImpl(origin: String, ident: Ident, privateIncludes: Seq[String], td: ast.TypeDef): Unit = {
    ...
    writeCFile(origin, ident.name + "_impl", "hpp", spec.cHeaderOutFolder.get, (w: IndentWriter) => {
      w.wl("#pragma once")
      w.wl
      ...
      w.wl(s"struct ${typeNameHolderStruct} : ::djinni::${holderCpp}<${selfCpp}>")
      w.bracedSemi {
  ...
  override def generateRecord(origin: String, ident: Ident, doc: Doc, params: Seq[TypeParam], r: ast.Record): Unit = {
    ...
    generateRefImpl(origin, ident, typeResolver.privateImports.toSeq, r)

    writeCFilePair(origin, ident, typeResolver.publicImports.toSeq, typeResolver.privateImports.toSeq, r)((w: IndentWriter) => {
      writeDoc(w, doc)
  ...
  override def generateInterface(origin: String, ident: Ident, doc: Doc, typeParams: Seq[TypeParam], i: Interface): Unit = {
    ...
    generateRefImpl(origin, ident, typeResolver.privateImports.toSeq, i)
    ...
    writeCFilePair(origin, ident, typeResolver.publicImports.toSeq, typeResolver.privateImports.toSeq, i)((w: IndentWriter) => {
      writeDoc(w, doc)
      w.wl(s"""typedef struct ${typeNameHolderStruct} * ${typeName};""")
